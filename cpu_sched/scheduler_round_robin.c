...
void stud_RR(struct run_queue* rq, enum events event, int pid) {
    switch(event) {
        case start:      stud_RR_start(rq, pid);     break;
        case terminate:  stud_RR_terminate(rq);      break;
        case clock_tick: stud_RR_clock_tick(rq);     break;
        case wait:       stud_RR_wait(rq);           break;
        case wake_up:    stud_RR_wake_up(rq, pid);   break;
